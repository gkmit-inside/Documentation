...
This Level 1 DFD expands on the **GKMIT INSIDE** platform — a role-based internal social network designed for company employees and administrators.  
...
At the core of the system lies the **GKMIT INSIDE** platform — the central application responsible for handling user actions, post management, engagement tracking, and analytics.  
There are two primary user roles:kmi
...
All employee actions are routed through the **GKMIT INSIDE System**, which controls business logic and data flow.

---

### **Admin**

The **Admin** has elevated privileges within the platform.

Admins can:

- Manage user accounts (approve/reject new users).
...
- Handles user registration, login, and access and refresh token generation.
...
- Employees can create their posts.
...
- Fetches data from **POSTS** and applies filters.
...
- Pulls and aggregates data from all collections for insights and reporting.
...
This **Level 1 Data Flow Diagram (DFD)** provides a holistic overview of the **GKMIT INSIDE** system’s data flow.
